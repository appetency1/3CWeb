...
function validateLogin() {
  ...
  if (!login.username.trim()) { loginErrors.username = '请输入用户名或手机号'; ok = false }
  if (!login.password) { loginErrors.password = '请输入密码'; ok = false }
  return ok
}
...
function validateRegister() {
  ...
  if (!reg.username.trim()) { regErrors.username = '请设置用户名'; ok = false }
  else if (reg.username.trim().length < 3) { regErrors.username = '用户名至少3个字符'; ok = false }

  if (!reg.password) { regErrors.password = '请设置密码'; ok = false }
  else if (reg.password.length < 6) { regErrors.password = '密码至少6位'; ok = false }

  if (!reg.confirmPwd) { regErrors.confirmPwd = '请确认密码'; ok = false }
  else if (reg.confirmPwd !== reg.password) { regErrors.confirmPwd = '两次密码不一致'; ok = false }

  if (reg.phone && !/^1[3-9]\d{9}$/.test(reg.phone)) { regErrors.phone = '手机号格式不正确'; ok = false }
  return ok
}
...
function switchMode() {
  ...
  regErrors.username = ''; regErrors.password = ''; regErrors.confirmPwd = ''; regErrors.phone = ''
  setTimeout(updateTabPill, 50)
}
...
onMounted(() => {
  ...
  const onMove = (e: MouseEvent) => {
    ...
    card!.style.transform = `rotateX(${(e.clientY - cy) / (rect.height / 2) * -3}deg) rotateY(${(e.clientX - cx) / (rect.width / 2) * 3}deg)`
  }
        ...
        <div ref="cardRef" class="card">
          ...
          <form class="form" :class="{ active: isLogin }" @submit.prevent="handleLogin">
            ...
            <div class="field">
              ...
              <button type="button" class="field__icon" @click="showPwd = !showPwd">
          ...
          <form class="form" :class="{ active: !isLogin }" @submit.prevent="handleRegister">
            ...
            <div class="field">
              ...
              <label class="field__label">密码</label>
              <button type="button" class="field__icon" @click="showRegPwd = !showRegPwd">
                <svg v-if="!showRegPwd" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                <svg v-else width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>
              </button>
            </div>
...
.form.active {
  display: block;
  animation: formIn 0.5s cubic-bezier(0.22, 1, 0.36, 1);
}
...
.btn:hover { box-shadow: 0 14px 36px rgba(176,92,79,0.22); transform: translateY(-2px); }
...
.success.show .success__ring svg {
  animation: popIn 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) 0.25s forwards;
}
